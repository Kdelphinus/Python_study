...
def process_collision():
    global curr_bubble, fire, curr_fire_count, wall_height
...
def visit(row_idx, col_idx, color=None):
    # 맵의 범위 안에 있는지 확인
    if not (0 <= row_idx < MAP_ROW_COUNT and 0 <= col_idx < MAP_COLUMN_COUNT):
        return

    # 현재 셀의 색상이 주어진 color와 같은지 확인
    if color and map[row_idx][col_idx] != color:
        return

    # 빈 공간이거나 존재할 수 없는 위치인지 확인
    ...
    visited.append((row_idx, col_idx))  # 방문 처리
...
def remove_not_visited_bubbles():
    # 붕 떠있는 버블만 확인
    bubbles_to_remove = [
        b for b in bubble_group if (b.row_idx, b.col_idx) not in visited
    ]
    for bubble in bubbles_to_remove:
        map[bubble.row_idx][bubble.col_idx] = "."
        bubble_group.remove(bubble)
...
def remove_hanging_bubbles():
    ...
    for col_idx in range(MAP_COLUMN_COUNT):
        if map[0][col_idx] != ".":
            visit(0, col_idx)  # 색깔 상관없이 방문하기 위해
...
def change_bubble_imgae(image):
    for bubble in bubble_group:
        bubble.image = image


def diplay_game_over():
...
while running:
    ...
    if not bubble_group:
        ...
        is_game_over = True
        change_bubble_imgae(bubble_images[-1])
...
pygame.time.delay(2000)  # 2000ms 대기
pygame.quit()
